    ...
    <div className="max-w-md mx-auto space-y-4">
      <div className="rounded-xl border border-black/10 bg-white p-5">
        <div className="mb-4">
          ...
          <p className="text-sm text-black/60">Sign in to manage your bots and ingestion.</p>
        </div>
        ...
        <form onSubmit={login} className="space-y-3">
          ...
          <button type="submit" className="w-full px-3 py-2 rounded-md bg-blue-600 text-white" disabled={loading}>{loading ? "Logging in..." : "Login"}</button>
        </form>
      </div>
      <p className="text-sm text-center">No account? <a href="/register" className="text-blue-600 hover:underline">Create one</a></p>
    </div>
  );
}
